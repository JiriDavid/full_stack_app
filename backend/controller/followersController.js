...
exports.getFollower = catchAsync(async(req, res, next)=>{
  ...
  const followers = await Followers.find({follower}).select("followee").populate("followee")
...
exports.getFollowee = catchAsync(async(req, res, next)=>{
  ...
  const followees = await Followers.find({followee}).select("follower").populate("follower")
